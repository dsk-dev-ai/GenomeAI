...
def migration() -> object:
    ...
    for path in versions_dir.iterdir():
        if path.name.endswith(".py") and "4a9d6aaf7429" in path.name:
            spec = util.spec_from_file_location("transcript_migration", path)
            assert spec is not None
            mod = util.module_from_spec(spec)
            spec.loader.exec_module(mod)  # type: ignore[union-attr]
            return mod
    msg = "migration 4a9d6aaf7429 not found"
...
def test_migration_down_revision(migration: object) -> None:
    assert migration.down_revision == "0e9a0c1aa7b6"  # type: ignore[union-attr]
