...
def migration() -> object:
    ...
    for path in versions_dir.iterdir():
        if path.name.endswith(".py") and "3c7e420e271b" in path.name:
            spec = util.spec_from_file_location("sample_migration", path)
            assert spec is not None
            mod = util.module_from_spec(spec)
            spec.loader.exec_module(mod)  # type: ignore[union-attr]
            return mod
    msg = "migration 3c7e420e271b not found"
...
def test_migration_down_revision(migration: object) -> None:
    assert migration.down_revision == "0f3f64decdb7"  # type: ignore[union-attr]
