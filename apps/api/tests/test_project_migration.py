...
def migration() -> object:
    ...
    for path in versions_dir.iterdir():
        if path.name.endswith(".py") and "7cf6d6506732" in path.name:
            spec = util.spec_from_file_location("project_migration", path)
            assert spec is not None
            mod = util.module_from_spec(spec)
            spec.loader.exec_module(mod)  # type: ignore[union-attr]
            return mod
    msg = "migration 7cf6d6506732 not found"
...
def test_migration_down_revision(migration: object) -> None:
    assert migration.down_revision == "df47a097676a"  # type: ignore[union-attr]
