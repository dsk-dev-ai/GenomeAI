...
def migration() -> object:
    ...
    for path in versions_dir.iterdir():
        if path.name.endswith(".py") and "ec292f5d7884" in path.name:
            spec = util.spec_from_file_location("gene_migration", path)
            assert spec is not None
            mod = util.module_from_spec(spec)
            spec.loader.exec_module(mod)  # type: ignore[union-attr]
            return mod
    msg = "migration ec292f5d7884 not found"
...
def test_migration_down_revision(migration: object) -> None:
    assert migration.down_revision == "3c7e420e271b"  # type: ignore[union-attr]
