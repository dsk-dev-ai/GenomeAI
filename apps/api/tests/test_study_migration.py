...
def migration() -> object:
    ...
    for path in versions_dir.iterdir():
        if path.name.endswith(".py") and "df47a097676a" in path.name:
            spec = util.spec_from_file_location("study_migration", path)
            assert spec is not None
            mod = util.module_from_spec(spec)
            spec.loader.exec_module(mod)  # type: ignore[union-attr]
            return mod
    msg = "migration df47a097676a not found"
...
def test_migration_down_revision(migration: object) -> None:
    assert migration.down_revision == "f90b9cf85ea9"  # type: ignore[union-attr]
