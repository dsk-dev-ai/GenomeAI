...
def migration() -> object:
    ...
    for path in versions_dir.iterdir():
        if path.name.endswith(".py") and "cc42f7960492" in path.name:
            spec = util.spec_from_file_location("protein_migration", path)
            assert spec is not None
            mod = util.module_from_spec(spec)
            spec.loader.exec_module(mod)  # type: ignore[union-attr]
            return mod
    msg = "migration cc42f7960492 not found"
...
def test_migration_down_revision(migration: object) -> None:
    assert migration.down_revision == "4a9d6aaf7429"  # type: ignore[union-attr]
